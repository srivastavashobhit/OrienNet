...
def cal_rotation_angle(angles_array):
...
def find_video_rotation(video_path):
...
def display_examples(model, input, num_images=5, size=None, crop_center=False,
    ...
    y_pred = np.argmax(model.predict(x), axis=1) #here
...
def get_frames_from_videos(path_to_video, num_extraction):
    # to-do use a key frame extractor here instead of a simply random
    required_frames_per_sec = 1
    rotation_angle = find_video_rotation(path_to_video)
...
def get_train_train_data(np_dataset_path, test_split_percentage):

    all_frames = np.load(np_dataset_path)
    test_split_index = all_frames.shape[0]*(1-test_split_percentage)
    test_split_index = int(test_split_index)

    train_frames = all_frames[:test_split_index]
    test_frames = all_frames[test_split_index:]
